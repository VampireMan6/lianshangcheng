<template>	<view class="box"><!-- 		<view class="page-name hide">			<text class="name">注册</text>			<view class="tips">欢迎来到 链++ </view>		</view> -->		<view class="form-cont">
		...
		methods: {
			go_register:function(){
				var self=this;
				user.user_register(self.mobile,self.code,self.pass1,self.pass2,self.invite,self.name,function(res) {
			...
			get_code:function(){
				var self=this;
				sms.send_sms(self.mobile,'1',function(res) {
					if (res.data.code == 200) {
						self.app._toast(res.data.message);
						self.movetime();
					}else{
