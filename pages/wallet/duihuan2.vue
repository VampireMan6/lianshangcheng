		...
		<view class="content bc-white">
			...
			<view class="pb-20">
				...
				<view class="Input-cont flex-center flex-j-between">
					<input type="number" placeholder="请输入回兑枚数" v-model="number" />
				</view>
			</view>
			<!-- <button class="btn mt-10 mb-30" @click="paySW=true">确认兑换</button> -->
			<button class="btn mt-10 mb-30" @click="submit">确认回兑</button>
		</view>
		<!-- 弹框 -->
		...
		computed:{
			...
			numberXiaoHao() {
				if(!this.number.trim()) {
					...
					return '回兑 ' +  (parseFloat(this.dataList[this.index].price) * this.number/parseFloat(this.coinMsg.price)).toFixed(2) + ' 链++';
				}
			},
			numberKeYong() {
				return this.dataList[this.index].currency_name + parseFloat(this.dataList[this.index].balance).toFixed(2)
			}
		},
		methods: {
			...
			goWithdraw:function(){
				...
				if(!self.load){
					self.app._toast("正在回兑中，请不要重复点击");
					return ;
				}
				self.load=false;
				uni.showNavigationBarLoading();
				uni.request({
					url: config.api_service + "/post.huidui",
					data: send,
					method: "post",
					header: {Authorization: config.getToken()},
					success: res => {
