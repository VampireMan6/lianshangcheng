	...
	<view class="box pl-15 pr-15 pb-15" @click="timeSW=false">
		<view style="position: relative;z-index: 1;">
			...
			<view class="balance-cont flex-center flex-j-between bc-white">
				<view class="cont text-center nowrap">
					<view class="font-12 font-light-gray">当月支出</view>
				...
				<view class="cont text-center nowrap">
					<view class="font-12 font-light-gray">当月收入</view>
			...
			<view class="cont-list flex-center flex-j-between" v-for="(item,index) in list" :key="index">
				...
				<view class="cont-data">
					<view class="flex-center flex-j-between">
						<view class="one-row" v-text="item.currency_name">BTC</view>
						<view class="font-w-b nowrap" v-text="app._toFixed(item.amount,2)">316546.015421</view>
					</view>
		...
		<view class="winPopup flex-row flex-j-end" v-if="popSW" @click="popSW=false">
			<view class="Popup-content bc-white pl-15 pr-15 pt-15" @click.stop="">
				<view class="pb-30">
					<view class="font-16 font-w-b text-left">资产类型</view>
					<view class="choice-list flex-center flex-j-between">
						<view class="name nowrap text-center" :class="{'active':currency_id==''}" @click="currency_id=''">全部</view>
						<view class="name nowrap text-center" :class="{'active':currency_id==item.currency_id}"
						 @click="currency_id=item.currency_id" v-for="(item,index) in dataList" :key="index" v-text="item.currency_name">链++</view>
					</view>
				</view>
				<view class="pb-30">
	...
	export default {
		...
		methods: {
			...
			reset:function(){
				var self=this;
				self.type_id="";//类型id
				self.currency_id="";//币种id
				self.type="";//1转入 2转出
				// self.time="";//月份
				self.time=self.getTime(0);//月份
				self.min_money="";//最小金额
			},
			get_data:function(){
				var self=this;
			...
			get_list:function(){
				...
				let send={
					...
					currency_id:self.currency_id,//币种id
				...
				uni.request({
					url: config.api_service + "/get.dh.bills",
					data: send,
					method: "get",
					header: {Authorization: config.getToken()},
					success: res => {
						...
						config.api_status(res);
						if (res.data.code == 200) {
							self.data.in=parseFloat(res.data.data.month_in).toFixed(2);
							self.data.out=parseFloat(res.data.data.month_out).toFixed(2);
			...
			get_type:function(){
				...
				uni.showNavigationBarLoading();
				uni.request({
					url: config.api_service + "/get.dh.bill.types",
					data: {},
					method: "get",
					header: {Authorization: config.getToken()},
					success: res => {
